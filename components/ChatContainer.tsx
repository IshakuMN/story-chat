...
export const ChatContainer: React.FC<ChatContainerProps> = ({ script }) => {
  ...
  const currentMessage = timeline.find(
    (msg) => frame >= msg.startFrame && frame < msg.typingEndFrame,
  );
  ...
  if (isMeTyping && currentMessage) {
    ...
    const durationFrames =
      currentMessage.typingEndFrame - currentMessage.startFrame;
    const elapsedFrames = frame - currentMessage.startFrame;
    const charsToShow = Math.floor(
      (elapsedFrames / durationFrames) * totalChars,
    );
  ...
  return (
    ...
    >
      ...
      {/* WhatsApp Header */}
      <div
        className="relative z-10 h-[88px] flex items-end pb-2 px-3"
      ...
      >
        <div className="flex items-center gap-2 w-full pb-2">
          ...
          <div className="flex-1 flex flex-col">
            ...
            <span className="text-[#FFFFFF] text-[13px] opacity-80">
              online
            </span>
          </div>
        ...
        <div className="py-3">
          ...
          <div className="flex justify-center mb-3">
            <div className="bg-[#FFFFFF] px-3 py-1 rounded-md shadow-sm">
              <span className="text-[#667781] text-[12.5px] font-medium">
                Today
              </span>
            </div>
          </div>
          ...
          {visibleMessages.map((msg, idx) => (
            <MessageBubble key={idx} message={msg} isMe={msg.from === me} />
          ))}

          {isOtherTyping && <TypingIndicator isMe={false} />}
        </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1 flex items-center gap-2 bg-white rounded-full px-3 py-2 min-h-[40px] shadow-sm">
              <button>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                  <path
                    d="M18.5 2h-13C4.12 2 3 3.12 3 4.5v15C3 20.88 4.12 22 5.5 22h13c1.38 0 2.5-1.12 2.5-2.5v-15C21 3.12 19.88 2 18.5 2zM12 6.5c1.38 0 2.5 1.12 2.5 2.5s-1.12 2.5-2.5 2.5-2.5-1.12-2.5-2.5 1.12-2.5 2.5-2.5zM17 17H7v-.75c0-1.5 3-2.25 5-2.25s5 .75 5 2.25V17z"
                    fill="#54656F"
                  />
                </svg>
              </button>
            ...
            <button
              className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${isMeTyping && typingText.length > 0 ? "bg-[#25D366]" : "bg-[#25D366]"}`}
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path
